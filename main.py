...
GROQ_API_KEY     = "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"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = bq(
        "📋 <b>All Commands</b>\n\n"
        "<b>FREE:</b>\n"
        "/ai — Super AI chat (FREE + can execute all tools!)\n"
        "/image — AI image gen (FREE)\n"
        "/nasaapod /nasaepic — NASA (FREE)\n"
        "/start — Main menu + daily credits\n\n"
        "<b>1 Credit each:</b>\n"
        "/userid — TG ID → phone\n"
        "/usernum — Username → phone\n"
        "/indinfo — Indian number info\n"
        ...
        "💡 <b>Tip:</b> /ai can do ALL of these for you!\n"
        "Just say: \"Check IP 8.8.8.8\" or \"What's the weather in Dhaka?\"\n\n"
        "👺 DEV @Shuvobhai ✅"
    )
...
async def cmd_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user["banned"]: return
    if not user.get("is_admin") and user["credits"] <= 0:
        sent = await update.message.reply_text(bq("❌ Insufficient credits.\n\n💡 Claim করো /start দিয়ে।"), parse_mode="HTML")
        schedule_delete(sent); return
...
async def handle_user_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if pending == PENDING_MKCODE_CREDITS:
        try:
            ...
            sent = await update.message.reply_text(bq("❌ Send a valid number."), parse_mode="HTML")
            schedule_delete(sent); return
        _mkcode_state[uid] = {"credits": credits}
        context.user_data["pending"] = PENDING_MKCODE_EXPIRY
        sent = await update.message.reply_text(
            bq(f"✅ Credits: <b>{credits}</b>\n\n⏰ Expiry in hours? Send <code>0</code> for no expiry:"),
            parse_mode="HTML"
        )
        schedule_delete(sent); return

    if pending == PENDING_MKCODE_EXPIRY:
        try:
            hours = int(text)
        except ValueError:
            sent = await update.message.reply_text(bq("❌ Send a valid number."), parse_mode="HTML")
        ...
        code_str = "SHUVO_" + "".join(random.choices(string.ascii_letters + string.digits + "!@#$", k=7))
        codes = load_codes()
        codes[code_str] = {
            "credits": credits,
            "expires_at": (datetime.now() + timedelta(hours=hours)).isoformat() if hours > 0 else None,
            "used_by": []
        }
        save_codes(codes)
        exp_text = f"{hours}h" if hours > 0 else "Never"
        sent = await update.message.reply_text(
            bq(f"🎫 <b>Code Created!</b>\n\n<code>{h(code_str)}</code>\n\n💎 Credits: <b>{credits}</b>\n⏰ Expires: <b>{exp_text}</b>"),
            parse_mode="HTML"
        )
        schedule_delete(sent); return
    ...
    try:
        async with httpx.AsyncClient(timeout=20) as client:
            if pending == PENDING_USERID:
                ...
                voice     = VOICE_MAP.get(lang_code, VOICE_DEFAULT)
                if not text:
                    await sent.edit_text(bq("❌ কোনো text পাইনি।"), parse_mode="HTML")
                    schedule_delete(sent); return
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "back_home":
        ...
        sent = await query.message.reply_text(bq("🎫 <b>Create Redeem Code</b>\n\n💎 How many credits?\nSend a number (e.g. <code>10</code>):"), parse_mode="HTML")
        schedule_delete(sent)
